...
- agents_in_AS.csv contains a row for every single ArchivesSpace Archival Object that uses an Agent. Update frequency: approximately monthly, to reflect new/updated finding aid data. This currently comes from a custom report, "Agents-Archival Objects", in ArchivesSpace. This report is: record type Agents, display Agent Name, link Archival Objects, sort by Agent Name, limit 200k (achieved by using the browser inspect tool to go over the set limit of 50k). To get a new csv: in ArchivesSpace run the report to csv. Open with a capable spreadsheet editor (not Microsoft Excel), remove the rows above the title row, and match the column order: name, archival_object_title, archival_object_ref_id. Delete all other columns.
